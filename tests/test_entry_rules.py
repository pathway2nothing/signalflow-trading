...
class TestSignalEntryRuleBasic:
    ...
    def test_fall_ignored_when_shorts_off(self):
        ...
        sigs = _signals(
            [{"pair": "BTCUSDT", "timestamp": TS, "signal_type": "fall", "signal": -1, "probability": 0.9}]
        )
        orders = rule.check_entries(sigs, {"BTCUSDT": 100.0}, _make_state())
        assert orders == []
    ...
    def test_fall_generates_sell(self):
        ...
        sigs = _signals(
            [{"pair": "BTCUSDT", "timestamp": TS, "signal_type": "fall", "signal": -1, "probability": 0.8}]
        )
        orders = rule.check_entries(sigs, {"BTCUSDT": 100.0}, _make_state())
        assert len(orders) == 1
    ...
    def test_none_signal_skipped(self):
        ...
        sigs = _signals(
            [{"pair": "BTCUSDT", "timestamp": TS, "signal_type": "none", "signal": 0, "probability": 0.0}]
        )
...
class TestSignalEntryRuleCapital:
    def test_min_order_notional_filters(self):
        ...
        sigs = _signals(
            [{"pair": "BTCUSDT", "timestamp": TS, "signal_type": "rise", "signal": 1, "probability": 1.0}]
        )
        assert rule.check_entries(sigs, {"BTCUSDT": 100.0}, _make_state()) == []
...
class TestSignalEntryRuleProbability:
    def test_min_probability_filter(self):
        ...
        sigs = _signals(
            [{"pair": "BTCUSDT", "timestamp": TS, "signal_type": "rise", "signal": 1, "probability": 0.5}]
        )
        assert rule.check_entries(sigs, {"BTCUSDT": 100.0}, _make_state()) == []
...
class TestFixedSizeEntryRule:
    def test_rise_signal_buy(self):
        ...
        sigs = _signals(
            [{"pair": "BTCUSDT", "timestamp": TS, "signal_type": "rise", "signal": 1}]
        )
        orders = rule.check_entries(sigs, {"BTCUSDT": 100.0}, _make_state())
        assert len(orders) == 1
        assert orders[0].side == "BUY"
    ...
    def test_fall_signal_sell(self):
        ...
        sigs = _signals(
            [{"pair": "BTCUSDT", "timestamp": TS, "signal_type": "fall", "signal": -1}]
        )
        orders = rule.check_entries(sigs, {"BTCUSDT": 100.0}, _make_state())
        assert len(orders) == 1
        assert orders[0].side == "SELL"
