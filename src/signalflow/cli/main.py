...
def _prod_conf(name: str) -> str:
    """The minimal sf-prod strategy conf that runs a saved V5 flow."""
...
def _promote_checklist(name: str) -> str:
    return (
        ...
        f"  1. copy flow.yaml + the model dir into <sf-prod>/flows/{name}/\n"
...
@main.command()
@click.argument("flow_yaml", type=click.Path(exists=True, dir_okay=False))
@click.option("--to", "target", required=True, help="sf-prod directory to promote into, or 'shadow' for a dry stub.")
...
def promote(flow_yaml: str, target: str, dry_run: bool, force: bool) -> None:
    """Validate FLOW_YAML's promotion evidence and generate the sf-prod strategy conf."""
    import json
