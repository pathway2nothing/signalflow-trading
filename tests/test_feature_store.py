...
def _ds():
    return sf.dataset("synthetic", pairs=["BTCUSDT"], start="2023-01-01", end="2023-03-01", interval="1h")


def _count_compute(monkeypatch):
    calls = {"n": 0}
    orig = FeaturePipeline.compute

    def counting(self, df):
        calls["n"] += 1
        return orig(self, df)

    monkeypatch.setattr(FeaturePipeline, "compute", counting)
    return calls
...
def test_second_fit_skips_recompute(tmp_path, monkeypatch):
    ...
    sf.ForecastModel(target=sf.FixedHorizon(12), features=sf.FeaturePipeline(sf.SMA(20))).fit(
...
def test_param_change_is_cache_miss(tmp_path, monkeypatch):
    ...
    ds = _ds()
    store = sf.FeatureStore(tmp_path)
    sf.ForecastModel(target=sf.FixedHorizon(12), features=sf.FeaturePipeline(sf.SMA(10))).fit(
        ds, feature_store=store
    )
    n1 = calls["n"]
    sf.ForecastModel(target=sf.FixedHorizon(12), features=sf.FeaturePipeline(sf.SMA(11))).fit(
        ds, feature_store=store
    )
    assert calls["n"] > n1
...
def test_span_change_is_cache_miss(tmp_path):
    store = sf.FeatureStore(tmp_path)
    pipe = sf.FeaturePipeline(sf.SMA(20))
    a = _ds()
    b = sf.dataset("synthetic", pairs=["BTCUSDT"], start="2023-01-01", end="2023-04-01", interval="1h")
